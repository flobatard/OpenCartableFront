...
- Replay : `maskAllInputs` + `REPLAY_MASK_SELECTOR` (sorties du modèle, corrigés, données du backoffice). Ce sélecteur vise des **classes de template** et échouerait en silence à un renommage — `analytics.masking.spec.ts` le garde, le mettre à jour avec les classes.
