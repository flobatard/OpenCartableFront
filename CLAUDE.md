...
| `features/` | Pages par domaine : `home/`, `auth/`, `onboarding/`, `profile/`, `settings/`, `subjects/`, `courses/` (liste, création, page cours à onglets, éditeurs de bloc et de module), `course-assistant/` (chat, panneau flottant, revues de proposition), `student/` (pages élèves), `search/`, `docs/`, `privacy/` (politique de confidentialité), `admin/` (backoffice : coquille à menu latéral, page des jobs de maintenance) |
...
- Guards : `TEACHER_GUARDS = [authGuard, onboardingGuard]` dans cet ordre ; jamais `onboardingGuard` sur la route onboarding elle-même ; guard et callback fail-open si l'API profil est injoignable. Seule exception : le backoffice (`ADMIN_GUARDS = [authGuard, superAdminGuard]`, sans onboarding) est **fail-closed**. Menu et guard ne font que masquer, par le helper pur `isSuperAdmin` appliqué au `platform_role` du profil — c'est le 403 de `/admin/*` qui barre.
...
- `replaceAll`/`replaceSelection` (et `insertAtCursor`, qui y délègue) éditent entre deux `pushUndoStop` (Ctrl-Z annule l'application d'une proposition ou une insertion) ; `writeValue` porte une garde anti-écho — le `setValue` du wrapper vide la pile d'annulation même à valeur identique (vérifié en vrai navigateur).
...
- Colonnes `::: columns` (ratio facultatif `1:2`/`2:1`) … `+++` … `:::` (`core/markdown/course-columns.ts`, découpage pur ligne à ligne, blocs de code opaques ; extension de bloc de `courseMarked` qui relexe chaque colonne par `blockTokens` en restaurant `lexer.state.top`) : marqueurs et ratios sont du contenu stocké — on en ajoute, on n'en retire ni n'en renomme. Hint `start` ancré sur `\n` et non validé, jamais `^` + drapeau `m`. Empilement intrinsèque (`flex-wrap`, largeurs minimales proportionnelles au ratio, seuil 26em — sous 54ch en serif), jamais de requête de largeur : le markdown vit aussi dans des panneaux plus étroits que le viewport ; ni `transform`, `filter`, `contain` ni `overflow` sur les colonnes (le plein écran d'un module est `position: fixed`).
...
- HITL : à l'événement `interrupt` le flux se ferme sans `done`, la proposition attend dans `pendingProposal`, la décision rouvre un flux ; la revue remplace l'éditeur par classe (Monaco survit), masquage piloté par `review()` (jamais `pending()`) ; `ProposalHost` (`core/course-assistant/proposal-host.ts`) orchestre. Mode « Édition auto » purement front (`ProposalModeService`, interrupteur du pied des chats d'édition) : l'effect de `ProposalHost` (dep `autoAccept`, construit en contexte d'injection) applique puis accepte sans revue, une tentative par proposition, repli sur la revue en cas d'échec ; `ALWAYS_REVIEWED_KINDS` (suppression de question, suppression de bloc) est toujours revu.
- Édition globale (`GlobalEditService`, `allow_edit` par tour via `turnOptions()`) : les propositions de sous-assistants (événements tagués `agent`, `pendingProposal.delegation`) ont pour hôte `CourseAssistantService.proposals` — revue dans `app-global-proposal-review` (`<dialog>` hébergé par `AssistantOutlet` **à côté** du panneau, `.global-proposal-review` masqué au replay), application sur la cible par l'éditeur monté (`TargetApplierRegistry`, inscription dans `BlockEditor`/`ModuleEditor`, flush avant la décision) sinon headless (`updateBlockContent`/`updateModule`, `applyExerciseProposal`), cible préchargée dès l'appel `edit_*` et original figé par proposition ; `ProposalHost.apply` peut être asynchrone ; l'activité `agent` alimente la carte de délégation et `foldTurnMessages` l'ignore (jamais persistée). Les propositions **structurelles** de l'assistant global (`block_add`/`block_delete`/`blocks_reorder`, `isStructureProposal`) n'ont ni `delegation` ni éditeur hôte : même fenêtre (`app-structure-proposal-review`, blocs figés à la revue), blocs courants lus dans `CourseService.detail()` si c'est ce cours, sinon par `fetchDetail` — jamais `loadDetail`, qui blanchirait la page cours —, application headless (`addBlock` → ressource ou module pointé → `reorderBlocks` ; `deleteBlock`, refusé tant que l'éditeur du bloc est inscrit au `TargetApplierRegistry` ; `reorderBlocks` sur l'ordre exact, helpers purs de `core/courses/structure-apply.ts`), carte du fil `reviewable` en mode global.
- Pièces jointes des chats (`core/course-assistant/attachment.model.ts`, `attachments.service.ts`, `image-downscale.ts`, `app-course-chat-attachments`) : brouillons dans `AssistantChatState.draftAttachments`, uploadés dès le choix du fichier (presign → PUT S3 → confirm) et rattachés par le back à l'envoi ; vidés au changement de conversation ou de vue, **jamais** par `#clearTurn`, et seulement une fois le flux ouvert dans `sendMessage` (un envoi raté reste réessayable). Le mime est **déduit de l'extension** quand `file.type` est vide ou vague (`.md`/`.csv` typés `text/plain`) : il est figé dans la signature du presign. Les images au-dessus du plafond sont réduites par canvas avant l'upload, et c'est `blob.type` qui fait foi (`toBlob` retombe sur PNG). Tout le style vit dans le sous-composant : `course-chat.scss` frôle le plafond d'erreur du budget `anyComponentStyle`.
...
- Replay : `maskAllInputs` + `REPLAY_MASK_SELECTOR` (sorties du modèle, corrigés, noms de fichiers joints, données du backoffice). Ce sélecteur vise des **classes de template** et échouerait en silence à un renommage — `analytics.masking.spec.ts` le garde, le mettre à jour avec les classes.
...
`docs/architecture.md` — Routing et rendu · Couche données · Espace cours · Éditeurs · Assistant IA · Markdown de cours · Impression et export HTML · Pages élèves et tuteur · Recherche · Réglages IA · Backoffice · Mesure d'audience et consentement · Design system et styles.
