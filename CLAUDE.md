...
- Encadrés `> [!DEFINITION]`… (`core/markdown/course-callouts.ts`, override `blockquote` du pipeline, pas un fence) : les mots-clés sont du contenu stocké, cités par le catalogue de l'assistant et la garde du cours d'exemple back — on en ajoute, on n'en retire ni n'en renomme jamais. Leurs titres par défaut suivent la langue de l'interface : passés par `markdown-view` (`calloutTitles`) et inclus dans sa clé de rendu.
