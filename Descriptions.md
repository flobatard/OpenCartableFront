...
- Le **texte de cours** (bloc `text`) est du **markdown simple stocké en JSONB**, pas de HTML brut → plus sûr, réindexable, exploitable par l'IA ; titres, paragraphes, encadrés (définition, à retenir, méthode, exemple, remarque, attention — syntaxe des alertes GitHub `> [!DEFINITION]`), mise en colonnes (deux contenus côte à côte, `::: columns` … `+++` … `:::`, ratio facultatif `1:2`/`2:1`) **et liens externes** sont couverts par le markdown (pas de types de blocs dédiés — l'ancien type `lien` a été supprimé).
...
| **J5 — IA** *(5 briques livrées)* | **Assistant de cours** : chat par cours à conversations persistées, agent + tools (lecture de blocs, extraction PDF, images, modules), citations de sources, **chats d'édition à flux HITL** — bloc texte (proposition en diff), exercice (propositions par question, revue structurée) et module (propositions par fichier, aperçu exécutant le code proposé), Appliquer/Rejeter dans l'éditeur ou mode « Édition auto » (application sans revue, sauf suppression de question ou de bloc), **questions de l'assistant au prof** dans tous les contextes (formulaire à la place du composer, refus possible), **édition globale** (l'assistant du cours délègue des modifications à des sous-assistants et propose d'ajouter, supprimer ou réordonner des blocs, revue dans une fenêtre globale, application sur la cible) — cf. §5.7 ; **tuteur d'exercice de l'élève connecté** (fil par question dans la page de résolution, corrigé révélé par le serveur) ; restent le chat élève anonyme, la vue prof des soumissions, la vectorisation (ChromaDB, si actée) et le RAG | Interroger, critiquer et synthétiser un cours avec l'IA |
